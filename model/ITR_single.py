...
class ITRSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ITRSelfOutput(nn.Module):
    """
    The residual connection is defined in ITRLayer instead of here (as is the case with other models), due to the
...
class ITRAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attention = ITRSelfAttention(config)
        self.output = ITRSelfOutput(config)
...
class ITRIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ITRLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.seq_len_dim = 1
        self.attention = ITRAttention(config)
        self.intermediate = ITRIntermediate(config)
        self.output = ITROutput(config)
...
class ITREncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([ITRLayer(config) for _ in range(config.num_hidden_layers)])
...
class ITRPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = ITRConfig
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, ITREncoder):
            module.gradient_checkpointing = value




class ITRPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
...
class ITRModel(ITRPreTrainedModel): 
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = ITREncoder(config)

        self.layernorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.pooler = ITRPooler(config) if add_pooling_layer else None
...
class ITRForImageAndTextRetrieval(ITRPreTrainedModel): 

    def __init__(self, config):
        super().__init__(config)

        self.vilt = ITRModel(config)

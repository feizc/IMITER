...
class ViltModel(ViltPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            return (sequence_output, pooled_txt, pooled_img) + encoder_outputs[1:]

        return BaseModelOutputWithPooling(
            last_hidden_state=sequence_output,
            pooler_output=(pooled_txt, pooled_img),
...
class ViltForImageAndTextRetrieval(ViltPreTrainedModel):
    def __init__(self, config):
        ...
        # Classifier head
        self.rank_output = nn.Linear(config.hidden_size * 2, 2)
    ...
    def forward(
        ...
        # print(logits.size())
        loss = torch.abs(text_f) * torch.abs(image_f) 
        loss = loss.sum() 

...
from model import ITRForImageAndTextRetrieval, ITRConfig, compute_image_text_retrieval_loss
...
def eval(model, data_loader, train_config): 
    ...
    for _b in tqdm(text_loader, desc="text prefetch loop"):
        text_preload.append(
            {
                "text_ids": _b["text_ids"].to(train_config.device),
                "text_masks": _b["text_masks"].to(train_config.device),
                "text_labels": _b["text_labels"].to(train_config.device),
    ...
    for _b in tqdm(image_loader, desc="rank loop"): 
        ...
        _, _c, _w, _h = pixel_values.shape 

        image_batch_score = list() 
        for text_batch in text_preload: 
            fblen = len(text_batch["text_ids"]) 
...
def main(): 
    ...
    model = ITRForImageAndTextRetrieval.from_pretrained(train_config.tokenizer_path) 
    ...
    for epoch in range(train_config.max_epoch): 
        ...
        metrics = eval(model, data_loader, train_config)
        print(metrics)
